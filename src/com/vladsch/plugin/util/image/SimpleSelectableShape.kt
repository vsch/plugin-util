...
class SimpleSelectableShape(val shapeType: ShapeType, rectangle: Rectangle, borderWidth: Int, borderColor: Color?, fillColor: Color?) : DrawingShape(rectangle, borderWidth, borderColor, fillColor), SelectableDrawableShape {
    ...
    override fun drawShape(surface: BufferedImage, isSelected: Boolean, dashPhase: Float): BufferedImage {
        ...
        if (isSelected) {
            ...
            return if (shapeType == ShapeType.OVAL) {
                val bufferedImageSel = ImageUtils.drawHighlightOval(surface, rectangle.intX0, rectangle.intY0, rectangle.intWidth, rectangle.intHeight, Color.WHITE, borderWidthSel, null)
                ImageUtils.drawOval(bufferedImageSel, rectangle.intX0, rectangle.intY0, rectangle.intWidth, rectangle.intHeight, null, borderWidthSel, DASHES, 1.5f * dashPhase)
            } else {
                val bufferedImageSel = ImageUtils.drawRectangle(surface, rectangle.intX0, rectangle.intY0, rectangle.intWidth, rectangle.intHeight, Color.WHITE, borderWidthSel, rectangle.intCornerRadius)
                ImageUtils.drawRectangle(bufferedImageSel, rectangle.intX0, rectangle.intY0, rectangle.intWidth, rectangle.intHeight, null, borderWidthSel, rectangle.intCornerRadius, DASHES, 1.5f * dashPhase)
            }
        } else {
            return if (shapeType == ShapeType.OVAL) {
